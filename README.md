...
面向对象、面向过程
    ...
    面向对象是，设计一个系统时，把系统拆分成若干个对象，每个对象负责自己要做的部分。
    
    面向过程时，设计系统时，把系统拆分成若干个过程依次解决。

面向对象的三大基本特征和五大基本原则

    三大特征：封装、继承、多态；
    封装：外部无法直接访问内部，只能通过有限的函数来执行功能。无法了解内部实现。
    继承：子类具有父类的变量和函数。子类是对父类的扩展。
...
值传递、引用传递
    ...
    值传递是变量里存储的是值（内容） 两个不同的变量可以具有相同的值。
    引用传递是变量里存储的是地址
为什么说Java中只有值传递

    Java 中只有 值传递 这一种方式，只不过对于引用类型来说， 
    传递的参数是对象的引用 罢了。
#### 封装、继承、多态
...
什么是多态、方法重写与重载
    ...
    实现方法1继承2接口
    方法重写：子父类出了同名的函数，这个我们就称作为方法的重写。
    方法重载在一个类中存在两个或者两个以上的同名函数,
    称作为方法重载。
